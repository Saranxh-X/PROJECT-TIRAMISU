      ...
      <div className="glass-card rounded-2xl p-6 border border-slate-800">
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 pb-6 border-b border-slate-800">
          ...
          <div className="flex items-center gap-4">
            <div className="flex flex-col items-center justify-center p-4 rounded-2xl bg-slate-900 border border-slate-800 w-28 h-28">
              ...
              <span className="text-[10px] text-slate-400">{reviewsList.length} ratings</span>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(product.aspectRatings || {}).map(([key, score]) => (
                  <div
                    key={key}
                    className="flex items-center justify-between gap-3 px-3 py-1.5 rounded-lg bg-slate-900/60 border border-slate-800 text-xs"
                  >
            ...
            {[5, 4, 3, 2, 1].map((star) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-2 text-xs py-1 px-2 rounded-lg transition-colors ${
                    isSelected ? "bg-amber-400/20 border border-amber-400/40" : "hover:bg-slate-900/60"
                  }`}
                >
                  <span className="w-4 font-bold text-slate-300">{star}★</span>
        ...
        <div className="pt-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-2 overflow-x-auto w-full sm:w-auto pb-1 sm:pb-0">
            ...
            {selectedStar && (
              <button
                ...
                className="text-xs px-2.5 py-1 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/30 flex items-center gap-1"
              >
          ...
          <button
            ...
            className="w-full sm:w-auto px-4 py-2 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white text-xs font-bold flex items-center justify-center gap-2 hover:opacity-90 shadow-lg shadow-indigo-600/30 transition-all cursor-pointer"
          >
      ...
      <div className="space-y-4">
        {filteredReviews.length === 0 ? (
          <div className="glass-card rounded-2xl p-8 text-center text-slate-400">
            <p className="text-sm font-medium">No reviews match the selected filter criteria.</p>
            <button
              onClick={() => {
          ...
          filteredReviews.map((rev) => (
            <div
              key={rev.id}
              id={rev.id}
              className="glass-card rounded-2xl p-5 border border-slate-800 transition-all duration-300"
            >
              <div className="flex items-start justify-between gap-3 mb-2">
                ...
                {rev.aspectTag && (
                  <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-slate-900 text-indigo-400 border border-indigo-900/50 uppercase">
                    {rev.aspectTag}
                  </span>
                )}
              </div>
                ...
                <div className="flex text-amber-400">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`w-3.5 h-3.5 ${
                        i < rev.rating ? "fill-amber-400 stroke-amber-400" : "stroke-slate-600"
                      }`}
                    />
                  ))}
                </div>
              ...
              {/* Text Content */}
              <p className="text-xs text-slate-300 leading-relaxed mb-4">{rev.text}</p>
              ...
              <div className="flex items-center justify-between text-xs text-slate-400 pt-3 border-t border-slate-800/60">
                <span className="text-[11px] font-mono text-slate-500">ID: #{rev.id}</span>
                <div className="flex items-center gap-3">
                  <span className="text-[11px]">Was this helpful?</span>
                  <button
                    onClick={() => handleVote(rev.id, "helpful")}
                    className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-900 hover:bg-slate-800 text-slate-300 transition-colors border border-slate-800 cursor-pointer"
                  >
                    <ThumbsUp className="w-3 h-3 text-emerald-400" />
                    <span className="font-semibold text-[11px]">{rev.helpfulCount}</span>
                  </button>
                  <button
                  ...
                  >
                    ...
                    <span className="font-semibold text-[11px]">{rev.unhelpfulCount}</span>
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="glass-panel w-full max-w-lg rounded-2xl p-6 border border-slate-700 shadow-2xl relative">
            ...
            <form onSubmit={handleSubmitReview} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full py-2.5 rounded-xl bg-indigo-600 text-white font-bold text-xs hover:bg-indigo-500 transition-colors shadow-lg shadow-indigo-600/30 cursor-pointer"
              >
                Publish Review
              </button>
            </form>
